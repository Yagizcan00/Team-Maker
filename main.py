...
df["Asıl Hücum Becerisi"] = df["Top Becerisi"] * multipliers["top_becerisi"] + df["Topsuz Becerisi"] * multipliers["topsuz_becerisi"]
df["Asıl Savunma Becerisi"] = df["Birebir Savunma"] * multipliers["birebir_savunma"] + df["Takım Savunma"] * multipliers["takim_savunma"]

# Pozisyona göre etiketleme
df["Pozisyon"] = df.apply(
    lambda row: "Savunma" if row["Asıl Savunma Becerisi"] > row["Asıl Hücum Becerisi"] else "Hücum",
    axis=1
)
...
def balance_teams(df, num_teams=2, min_defenders=3, min_attackers=3):
    ...
    df_sorted = df.sort_values(by=["Asıl Hücum Becerisi", "Asıl Savunma Becerisi"], ascending=False)
